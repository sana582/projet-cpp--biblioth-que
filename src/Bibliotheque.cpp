...
int main() {
    ...
    // 1) On crée deux bibliothèques (pour tester aussi les échanges/ prêts)
    ...
    // On inscrit A001 dans b1 (cas normal)
    b1.inscrire(&a1);

    // On inscrit A002 dans b2 (cas normal)
    b2.inscrire(&a2);
    ...
    // On essaye d'ajouter un livre avec un code déjà existant (R001) :  refus
    ...
    // On affiche tous les livres pour vérifier que l'ajout a bien marché
    b1.afficherLivres(std::cout);

    std::cout << "\n\n - AFFICHAGE PAR CATEGORIE (B1) : \n";

    // On affiche par catégorie (roman, BD, poésie, ...)
    ...
    // Cas refus : désinscrire un adhérent inexistant
    std::cout << "On essaye de desinscrire A999 (inexistant)\n";
    std::cout << "Resultat: " << (b1.desinscrire("A999") ? "OK" : "REFUSE") << "\n";

    // Cas refus : désinscrire A001 alors qu'il a encore un livre emprunté
    std::cout << "On essaye de desinscrire A001 (il a encore R001)\n";
    std::cout << "Resultat: " << (b1.desinscrire("A001") ? "OK" : "REFUSE") << "\n";
    ...
    // Maintenant on désinscrit A001 (normalement ça passe)
    std::cout << "On desinscrit A001 apres qu'il ait tout rendu\n";
    std::cout << "Resultat: " << (b1.desinscrire("A001") ? "OK" : "REFUSE") << "\n";

    // Et on re-teste : une deuxième fois ça doit être refusé
    std::cout << "On essaye de desinscrire A001 encore une fois\n";
    std::cout << "Resultat: " << (b1.desinscrire("A001") ? "OK" : "REFUSE") << "\n";
